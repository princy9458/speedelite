  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <h1 className="text-4xl font-serif gold-text">Event Analytics</h1>
      </div>

      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        <GlassCard className="p-6">
          <p className="text-white/50 text-sm uppercase tracking-wider mb-2">Total Revenue</p>
          <p className="text-3xl font-serif">EUR {totalRevenue}</p>
        </GlassCard>
        <GlassCard className="p-6">
          <p className="text-white/50 text-sm uppercase tracking-wider mb-2">Total Bookings</p>
          <p className="text-3xl font-serif">{totalBookings}</p>
        </GlassCard>
        <GlassCard className="p-6">
          <p className="text-white/50 text-sm uppercase tracking-wider mb-2">Published Events</p>
          ...
          <tbody>
            {eventStats.map((stat) => (
              <tr key={stat._id} className="border-t border-white/5">
                ...
                <td className="p-4 text-white/60">{new Date(stat.date).toLocaleDateString()}</td>
