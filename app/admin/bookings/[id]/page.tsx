...
export default function BookingDetailPage() {
  ...
  const { lang } = useAdminUiStore();
  const t = getDictionary(lang).admin.bookings;
  ...
  const eventTitle = booking?.event
    ? lang === 'hr'
      ? booking.event.translations?.hr?.title || booking.event.title || '-'
      : booking.event.translations?.en?.title || booking.event.title || '-'
    : '-';

  const eventLocation = booking?.event
    ? lang === 'hr'
      ? booking.event.translations?.hr?.location || booking.event.location || '-'
      : booking.event.translations?.en?.location || booking.event.location || '-'
    : '-';
          ...
          <GlassCard className="p-6">
            ...
            <div className="space-y-4">
              ...
              <DetailItem
                ...
                value={
                  booking.event?.date
                    ? `${new Date(booking.event.date).toLocaleDateString(locale)} ${booking.event.time || ''}`
                    : '-'
                }
              />
