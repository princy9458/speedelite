    ...
    <div className="space-y-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <h1 className="text-4xl font-serif gold-text">Analytics Overview</h1>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        <StatCard label="Total Events" value={overview?.totals.events ?? 0} />
        <StatCard label="Total Customers" value={overview?.totals.customers ?? 0} />
        <StatCard label="Total Bookings" value={overview?.totals.bookings ?? 0} />
      </div>

      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[2fr_1fr]">
        <GlassCard className="p-6">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-serif">Bookings Over Time</h2>
            <span className="text-xs uppercase tracking-[0.2em] text-white/40">Last 6 months</span>
          </div>
        ...
        <GlassCard className="p-6">
          <h2 className="text-xl font-serif">Revenue By Event</h2>
          <div className="mt-6 h-64">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <GlassCard className="p-6">
        ...
        <div className="mt-6 space-y-4">
          {(overview?.recentActivity || []).length === 0 ? (
            <p className="text-white/50">No recent bookings yet.</p>
          ) : (
              ...
              >
                <div>
                  ...
                  <p className="text-lg font-medium">
                    {activity.user?.firstName || 'Guest'} {activity.user?.lastName || ''}
                  </p>
                  <p className="text-sm text-white/50">
                    {activity.event?.title || 'Event'} - {activity.event?.date ? new Date(activity.event.date).toLocaleDateString() : 'TBD'}
                  </p>
                </div>
