    ...
    <div className="min-h-screen bg-[#0B0B0B] text-white flex items-center justify-center px-6">
      <div className="glass-card border border-white/10 rounded-2xl p-8 w-full max-w-md space-y-6">
        <div>
          ...
          <h1 className="text-3xl font-serif gold-text">{t('loginTitle')}</h1>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <label className="text-xs uppercase tracking-[0.2em] text-white/50">{t('usernameLabel')}</label>
            <input
              type="email"
              value={email}
          ...
          <div className="space-y-2">
            <label className="text-xs uppercase tracking-[0.2em] text-white/50">{t('passwordLabel')}</label>
            <input
              type="password"
              value={password}
          ...
          >
            {isSubmitting ? t('loggingInBtn') : t('loginBtn')}
          </button>
        </form>
      </div>
    </div>
